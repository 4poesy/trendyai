      ...
      {clients.length === 0 ? (
        ...
        <div className="space-y-4">
          {clients.map(client => (
            <div 
              key={client.id} 
              className="p-5 bg-bg-panel border border-border-main rounded-lg flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-primary/50 transition-colors"
            >
              {/* Account details */}
              <button
                type="button"
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-text-main text-base hover:text-primary transition-colors flex items-center gap-2">
                      {client.name}
                ...
                {client.briefing && (
                  <div className="text-xs text-text-sub mt-3 bg-bg-card border border-border-main/50 px-3 py-2 rounded font-sans italic">
                    <span className="font-bold not-italic text-text-muted uppercase text-[9px] block mb-0.5">Briefing objective:</span>
                    "{client.briefing}"
                  </div>
                )}
              </button>
